...
void arm (num, secs)
	...
	unsigned int secs;
{
	HANDLE stdhnd;                                             //定义局部句柄变量：stdhnd
	stdhnd = GetStdHandle(STD_OUTPUT_HANDLE);                  //获取标准输出句柄

	/*=====================================调试信息 ========================================*/
#ifdef DEBUG
	if (stdhnd != INVALID_HANDLE_VALUE)
		printf("handle is %ld/n", stdhnd);
	else
		printf("failed to get stdhndl/n");
#endif
	/*======================================调试结束=======================================*/
...
/*===========================@@@@@@@@@@@@@@@@@@@@@@@@======Hmalloc函数====================================*/
/* Hmalloc :
   此函数功能, 四舍五入到4的倍数, 并且预置为"0",返回分配的内存地址指针.  Either succeeds
...
/*================================================== main入口函数 : 接下来我们将所有的功能都综合起来 =====================================*/
//本函数功能如下：
//1：程序入口
//2：将所有功能函数汇总在一起提供客户使用
/*=========================================================================================================================================*/
main (argc, argv)
	int argc;
	char ** argv;
{
	...
	HINF * wherefrom = NULL;
	/*=======================================================================================*/
	IA * ouraddr = NULL;                           //IA in address
	IA * themaddr = NULL;  
	/*======================================================================================*/
	USHORT o_lport = 0;
	USHORT ourport = 0;       //#define USHORT unsigned short
...
recycle:     //=========================开始recycle循环====================================
	...
	/* ===================如果没有参数给出，则在标准输入流中获取参数，并进行结构化参数.======================== */
	if (argc == 1) {   //第一个参数为“程序”本身名字                                                              
		cp = argv[0];    //char * cp;
		...
		insaved = read (0, cp, BIGSIZ);               //unsigned int insaved = 0      /* we're gonna fake fgets() here */
		if (insaved <= 0){
			bail ("wrong");
		}
...
{
	o_verbose = 1;
	holler ("[Windows下1.10版 NT 如下是常用的两个用法:]\n"
			"连接指定到某个IP: nc [-参数] <IP地址或者URL> <端口号1> [端口号2] ... \n"
			"使用某个端口侦听: nc -l -p <端口> [参数] [IP地址或者URL] [端口]\n"
			"参数表:");
	holler ("\
			-d      脱离命令行窗口，进入后台工作/n");
...
#endif
	holler ("-g 网管     source-routing hop point[s], up to 8 \n"
			...
			"-s 地址  本地源地址");
#ifdef TELNET
	holler (" -t  answer TELNET negotiation");
#endif
	holler ("\n\
			-u  UDP 模式下工作/n/\
